...
\multirow{2}{*}{\parbox[t]{2cm}{Availability}}
   & \multirow{2}{*}{\parbox[t]{25mm}{H/W or S/W failure}}
      & \parbox[t]{5cm}{Power outage: requests are redirected to an alternative
         server in less than 10 seconds}
...
\parbox[t]{2cm}{Usability}
   & \parbox[t]{25mm}{User wants to change tile size}
   & \parbox[t]{5cm}{The grid size is changed within 2 minutes of
      experimentation}
   & \parbox[t]{12mm}{(M, L)} \\
\hline
...
In this section we briefly discuss the limitations that have been considered
